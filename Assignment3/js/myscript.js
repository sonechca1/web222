...
window.onload = function() {
    var mLeafs = document.querySelector("#mLeafsTO");
    var tBody = document.querySelector("#tableBody");


    mLeafs.innerHTML = "<img src='" + logoImage.url + "' alt='" 
                        + logoImage.alt + "' width='" + logoImage.height + "'/>";

    for (var i = 0; i < players.length; ++i) {
        tBody.innerHTML += "<tr>" +
            "<td>" + players[i].number + "</td>" +
            "<td>" + "<img src='photo/country/" + players[i].nationCode + ".png' />" + "</td>" +
